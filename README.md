...
- **阈值报警**：每个传感器具备上下限；超限时持久化报警并通过 WebSocket 推送。同一传感器持续超限只保留一条未恢复报警（自动更新最新值、级别与连续次数），回到阈值内自动恢复并记录恢复值与恢复时间；人工确认仅对未恢复报警生效，重复确认或确认已恢复报警返回 409 业务冲突。报警列表展示确认/恢复状态与连续次数。
